...
public class SettingsFragment extends PreferenceFragmentCompat implements OnSharedPreferenceChangeListener{
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        for (int i = 0; i < preferenceScreen.getPreferenceCount(); i++) {
            ...
            if (!(preference instanceof CheckBoxPreference)) {
                String colorValue = sharedPreferences.getString(preference.getKey(), "");
                setPreferenceSummary(preference, colorValue);
            }
        }
    ...
    // of Preference.
    private void setPreferenceSummary(Preference preference, String colorValue) {
        if (preference instanceof ListPreference) {
            int indexOfColor = ((ListPreference) preference).findIndexOfValue(colorValue);
            CharSequence colorLabel = ((ListPreference) preference).getEntries()[indexOfColor];
            preference.setSummary(colorLabel);
        }
    }
